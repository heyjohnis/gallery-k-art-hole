...
const MyPageInfo = ({ user }) => {
  ...
  const currentYearlyCount = (yearlyCount - useYearlyCount) < 0 ? 0 : yearlyCount - useYearlyCount;
  const currentMonthlyCount = (monthlyCount - useMonthlyCount) < 0 ? 0 : monthlyCount - useMonthlyCount;
      ...
      <section>
        <div className="container">
          ...
          <div className={`row ${styles.point}`}>
            <div className="col-6">
              <div className={`row ${styles.item_txt} ${styles.border}`}>
                <p>잔여 이용 혜택</p>
                <h3 className={styles.info_data}>월 {currentMonthlyCount} 회 / 연 {currentYearlyCount}회</h3>
              </div>
            </div>
            <div className="col-6">
              <div className={styles.item_txt}>
                <p>연간 잔여 포인트 <span className={styles.smaller}>/ 연간 총 포인트</span></p>
                <h3 className={styles.info_data}>{commaFormat(currentPoint)}P <span className={styles.smaller}>/ {commaFormat(yearlyPoint)}P</span></h3>
              </div>
            </div>
          </div>
          ...
          <div className={`${styles.user_point} ${styles.black}`}>
              ...
              <p>최대 월 {monthlyCount}회 / 연 {yearlyCount}회</p>
          </div>
